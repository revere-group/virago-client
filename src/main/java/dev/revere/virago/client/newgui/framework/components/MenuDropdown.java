...
public class MenuDropdown extends MenuComponent {
	...
	public void onRender() {
		...
		DrawImpl.drawRect(x + 1, y + 1, width - 2 + arrowOffset, height - 1, backgroundColor);
			...
			for(int i = 0; i < values.length; i++) {
				...
				if (mouseX >= x + 1 && mouseX <= x + width - 1) {
					if (mouseY >= y && mouseY <= y + sHeight) {
						...
						DrawImpl.drawRect(x + 1, y, width - 1, sHeight, getColor(DrawType.BACKGROUND, ButtonState.HOVERACTIVE));
