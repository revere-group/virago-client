package dev.revere.virago.client.newgui.clickgui.pages;
...
import dev.revere.virago.client.newgui.Page;
import dev.revere.virago.client.newgui.clickgui.components.mods.*;
...
public class ModsPage extends Page {
	...
	public void onLoad() {
		...
		for (EnumModuleType category : EnumModuleType.values()) {
			MenuButton comp = new ModCategoryButton(category, x - 20, y, (int) (35 + fontService.getProductSans28().getStringWidth(category.getName())), height) {
				...
				public void onAction() {
					for (dev.revere.virago.client.newgui.framework.MenuComponent component : menu.getComponents()) {
		...
		if (activeModule == null) {
			...
			List<dev.revere.virago.client.newgui.framework.MenuComponent> toAdd = new ArrayList<>();
			...
			dev.revere.virago.client.newgui.framework.MenuComponent last = null;

			for (dev.revere.virago.client.newgui.framework.MenuComponent component : toAdd) {
	...
	private void initModPage(ModScrollPane pane) {
		...
		for (dev.revere.virago.client.newgui.framework.MenuComponent c : menu.getComponents()) {
