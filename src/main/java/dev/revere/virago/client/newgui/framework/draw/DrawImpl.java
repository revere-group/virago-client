...
public interface DrawImpl {
	...
	final Color primaryColor = Virago.getInstance().getServiceManager().getService(ModuleService.class).getModule(HUD.class).customColor1.getValue();
	final Color secondaryColor = Virago.getInstance().getServiceManager().getService(ModuleService.class).getModule(HUD.class).customColor2.getValue();
	...
	default void drawText(String text, int x, int y, int color){
		Virago.getInstance().getServiceManager().getService(FontService.class).getProductSans().drawString(text, x, y + 1, color);
	}
	...
	default void drawShadowUp(int x, int y, int width) {
		...
		drawGradientRectUpwards(x, y - DrawImpl.SHADOW_SIZE, width, SHADOW_SIZE, secondaryColor.getRGB(), secondaryColor.getRGB());
	}
	...
	default void drawShadowDown(int x, int y, int width) {
		...
		drawGradientRectUpwards(x, y, width, SHADOW_SIZE, secondaryColor.getRGB(), secondaryColor.getRGB());
	}
	...
	default void drawShadowLeft(int x, int y, int height) {
		...
		drawGradientRectSideways(x - DrawImpl.SHADOW_SIZE, y, SHADOW_SIZE, height, secondaryColor.getRGB(), secondaryColor.getRGB());
	}
	...
	default void drawShadowRight(int x, int y, int height) {
		...
		drawGradientRectSideways(x, y, SHADOW_SIZE, height, secondaryColor.getRGB(), secondaryColor.getRGB());
	}
