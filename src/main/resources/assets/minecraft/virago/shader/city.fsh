...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float time2 = lightSpeed * time;
    color += addLight(eye.xyz, ray.xyz, res.x, time2, lightHeight - 0.6);
    color += addLight(eye.yxz, ray.yxz, res.x, time2, lightHeight - 0.4);
    color += addLight(vec3(-eye.xy, eye.z), vec3(-ray.xy, ray.z), res.x, time2, lightHeight - 0.2);
    color += addLight(vec3(-eye.yx, eye.z), vec3(-ray.yx, ray.z), res.x, time2, lightHeight);
